...
impl Tool for HomeAssistantTool {
    ...
    fn execute(&self, args: &str) -> Result<String> {
        ...
        if trimmed == "--list" || trimmed.is_empty() {
            ...
            Ok(format!("Toggling device: {}\n\nNote: Requires HASS_URL and HASS_TOKEN environment variables.", device))
        } else if trimmed.starts_with("--set ") {
            let rest = trimmed.strip_prefix("--set ").unwrap_or("").trim();
            Ok(format!("Setting device state: {}\n\nNote: Requires HASS_URL and HASS_TOKEN environment variables.", rest))
        } else {
            Ok("Usage: homeassistant --list | --toggle <device_id> | --set <device_id> <state>".to_string())
        }
    }
}
...
impl Tool for SpotifyTool {
    ...
    fn execute(&self, args: &str) -> Result<String> {
        ...
        if trimmed.starts_with("--play ") {
            ...
            Ok(format!("Spotify play request: {}\n\nNote: Requires Spotify credentials. Set SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET.", query))
            ...
            Ok(format!("Spotify queue request: {}\n\nNote: Requires Spotify credentials.", track))
