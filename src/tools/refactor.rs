        ...
        match operation {
            "extract_function" => {
                ...
                let refactor_result = RefactorResult {
                    ...
                    changes,
                    message: if result.is_null() {
                        "No extract function action available".to_string()
                    } else {
                        "Extract function computed".to_string()
                    },
                };
                Ok(serde_json::to_string_pretty(&refactor_result)?)
            }
            "rename_symbol" => {
                let new_name =
                    new_name.ok_or_else(|| anyhow::anyhow!("rename_symbol requires new_name"))?;
                let params = serde_json::json!({
                    "textDocument": { "uri": uri },
            ...
            "inline_variable" => {
                ...
                let refactor_result = RefactorResult {
                    ...
                    changes,
                    message: if result.is_null() {
                        "No inline variable action available".to_string()
                    } else {
                        "Inline variable computed".to_string()
                    },
                };
                Ok(serde_json::to_string_pretty(&refactor_result)?)
            }
