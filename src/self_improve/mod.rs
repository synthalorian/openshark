...
use crate::memory::{MemoryStore, Session, SessionQualityMetrics, ModelTrendData};
...
fn analyze_sessions(memory: &MemoryStore, sessions: &[Session]) -> Result<Analysis> {
    ...
    let recommendations = generate_recommendations(
        &model_performance, &tool_performance, &model_trends, &tool_failure_patterns, &common_errors, &session_quality);
    let config_optimizations = generate_config_optimizations(&model_performance, &tool_performance, &prompt_effectiveness);
...
fn compute_model_performance(memory: &MemoryStore, sessions: &[Session]) -> Result<Vec<ModelStats>> {
    ...
    for session in sessions {
        ...
        let entry = model_stats.entry(session.model.clone()).or_insert((0, 0, 0));
        entry.0 += 1;
    ...
    let model_performance: Vec<ModelStats> = model_stats.iter()
...
fn compute_trend_for_model(model: &str, trends_data: &[ModelTrendData]) -> (TrendDirection, f64) {
    let model_trends: Vec<&ModelTrendData> = trends_data.iter()
...
fn compute_tool_performance(memory: &MemoryStore, sessions: &[Session]) -> Result<Vec<ToolStats>> {
    ...
    let tool_performance: Vec<ToolStats> = tool_stats.iter()
...
fn compute_model_trends(memory: &MemoryStore) -> Result<Vec<ModelTrend>> {
    ...
    for (model, data_points) in by_model {
        ...
        let trend_points: Vec<TrendDataPoint> = sorted.iter().map(|d| TrendDataPoint {
            day: d.day.clone(),
...
fn compute_prompt_effectiveness(memory: &MemoryStore) -> Result<Vec<PromptEffectiveness>> {
    ...
    let effectiveness: Vec<PromptEffectiveness> = raw_data.into_iter()
...
fn compute_tool_failure_patterns(memory: &MemoryStore) -> Result<Vec<ToolFailurePattern>> {
    ...
    let patterns: Vec<ToolFailurePattern> = raw_patterns.into_iter()
...
fn compute_common_errors(memory: &MemoryStore) -> Result<Vec<CommonError>> {
    ...
    let errors: Vec<CommonError> = raw_errors.into_iter()
        .map(|(msg, count)| {
...
fn calculate_quality_score(metrics: &SessionQualityMetrics) -> f64 {
    ...
    let volume_bonus = if metrics.tool_call_count >= 5 { 5.0 } else { 0.0 };
...
fn generate_recommendations(
    ...
    let high_volume_tools: Vec<_> = tool_performance.iter()
        .filter(|t| t.invocations >= 5)
        .collect();
    
    if !high_volume_tools.is_empty() {
        let avg_success = high_volume_tools.iter().map(|t| t.success_rate).sum::<f64>() / high_volume_tools.len() as f64;
        let below_avg: Vec<_> = high_volume_tools.iter()
    ...
    let poor_sessions: Vec<_> = session_quality.iter()
...
fn generate_config_optimizations(
    ...
    if let Some(best) = best_model {
        optimizations.push(format!(
            ...
            best, model_performance.iter().find(|m| m.model == best).map(|m| m.success_rate).unwrap_or(0.0)
        ));
    }
    
    let underperforming: Vec<_> = tool_performance.iter()
    ...
    if !prompt_effectiveness.is_empty() {
        let best_prompt = prompt_effectiveness.iter()
            .max_by(|a, b| a.success_rate.partial_cmp(&b.success_rate).unwrap_or(std::cmp::Ordering::Equal));
...
fn store_analysis_results(memory: &MemoryStore, analysis: &Analysis) -> Result<()> {
    for stat in &analysis.model_performance {
        let value = format!(
            ...
            stat.sessions, stat.tool_success, stat.tool_total, stat.success_rate, stat.trend, stat.trend_delta
        );
    ...
    let avg_quality = if !analysis.session_quality.is_empty() {
        analysis.session_quality.iter().map(|s| s.quality_score).sum::<f64>() / analysis.session_quality.len() as f64
    } else {
        0.0
    };
    memory.save_analysis_result("summary", "average_quality_score", &format!("{:.2}", avg_quality))?;
    
    let total_sessions = analysis.session_quality.len();
    memory.save_analysis_result("summary", "total_sessions_analyzed", &total_sessions.to_string())?;
...
fn print_analysis(analysis: &Analysis, config: &Config) {
    ...
    println!("{:<20} | {:8} | {:8} | {:8} | {:6} | {:12}", "Model", "Sessions", "Success", "Total", "Rate", "Trend");
    println!("{}", "-".repeat(75));
    for stat in &analysis.model_performance {
        ...
        println!("{:<20} | {:8} | {:8} | {:8} | {:5.1}% | {}",
            stat.model, stat.sessions, stat.tool_success, stat.tool_total, stat.success_rate, trend_str);
    }
    println!();
    
    println!("Tool Performance:");
    println!("{:<15} | {:8} | {:8} | {:6}", "Tool", "Calls", "Success", "Rate");
    println!("{}", "-".repeat(45));
    for stat in &analysis.tool_performance {
        println!("{:<15} | {:8} | {:8} | {:5.1}%",
    ...
    if !analysis.model_trends.is_empty() {
        ...
        for trend in &analysis.model_trends {
            ...
            println!("  {}: {} ({:.2} pp/day over {} days)",
                trend.model, direction, trend.improvement_rate, trend.data_points.len());
        }
        println!();
    }
    ...
    if !analysis.prompt_effectiveness.is_empty() {
        ...
        for (i, pe) in sorted.iter().take(3).enumerate() {
            println!("  {}. {:.1}% success over {} calls - {}",
                i + 1, pe.success_rate, pe.total_calls, pe.prompt_preview);
        }
        println!();
    }
    ...
    if !analysis.session_quality.is_empty() {
        ...
        let excellent = analysis.session_quality.iter().filter(|s| s.quality_label == QualityLabel::Excellent).count();
        let good = analysis.session_quality.iter().filter(|s| s.quality_label == QualityLabel::Good).count();
        let average = analysis.session_quality.iter().filter(|s| s.quality_label == QualityLabel::Average).count();
        let poor = analysis.session_quality.iter().filter(|s| s.quality_label == QualityLabel::Poor).count();
        println!("  Excellent: {} | Good: {} | Average: {} | Poor: {}", excellent, good, average, poor);
        println!();
    }
    ...
    let best_model = analysis.model_performance.iter()
        .max_by(|a, b| a.success_rate.partial_cmp(&b.success_rate).unwrap_or(std::cmp::Ordering::Equal))
        .map(|s| s.model.clone());
    
    if let Some(best) = best_model {
        if best != config.default_model {
            println!("  Consider setting default model to '{}' (highest success rate)", best);
        } else {
...
mod tests {
    ...
    fn test_compute_tool_performance() {
        ...
        let terminal = performance.iter().find(|p| p.tool_name == "terminal").unwrap();
    ...
    fn test_compute_session_quality() {
        ...
        assert!(sess1.quality_label == QualityLabel::Excellent || sess1.quality_label == QualityLabel::Good);
    }
    ...
    fn test_generate_recommendations() {
        ...
        let recs = generate_recommendations(&model_perf, &tool_perf, &trends, &patterns, &errors, &quality
        );
    ...
    fn test_empty_sessions_handling() {
        ...
        assert!(analysis.tool_performance.is_empty());
        assert!(!analysis.recommendations.is_empty());
        assert_eq!(analysis.recommendations[0], "All systems performing within expected parameters.");
    }
    ...
    fn test_tool_failure_patterns() {
        ...
        let pattern = patterns.iter().find(|p| p.tool_a == "fs" && p.tool_b == "terminal");
    ...
    fn test_common_errors() {
        ...
        let perm_error = errors.iter().find(|e| e.error_message.contains("permission denied"));
    ...
    fn test_generate_recommendations_empty() {
        ...
        assert_eq!(recs[0], "All systems performing within expected parameters.");
    }
