...
fn default_hermes_home() -> String { "~/.hermes".to_string() }
...
pub fn sync_pull(hermes_home: &str) -> anyhow::Result<String> {
    ...
    if !memory_src.exists() {
        anyhow::bail!("Hermes memory directory not found: {}", memory_src.display());
    }
...
pub fn sync_push(hermes_home: &str) -> anyhow::Result<String> {
    ...
    if !skills_dst.exists() {
        anyhow::bail!("Hermes skills directory not found: {}", skills_dst.display());
    }
